...
/*
 * Using sqrtf() for double arguments, instead of sqrt(double),
 * at least when math.h provides this.
 */
//#include <vxl_config.h>
#if ! VXL_C_MATH_HAS_SQRTF
#define sqrtf(x) (double)sqrt((double)(x))
#endif
...
void add_quats(const double q1[] , const double q2[] , double dest[]) {
	static int count = 0;
 ...
 *
 */
void build_rotmatrix(double m[] , double q[4]) {
...
void 
build_rotmatrix33(double m[], double q[4]){
	m[0] = 1.0 - 2.0 * (q[1] * q[1] + q[2] * q[2]);
	m[1] = 2.0 * (q[0] * q[1] - q[2] * q[3]);
	m[2] = 2.0 * (q[2] * q[0] + q[1] * q[3]);
